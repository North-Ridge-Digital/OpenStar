...
_EnglishFcstSkyConditionTable = _EnglishFcstSkyConditionTable
import SkyConditionCodes
_EnglishObsSkyConditionTable = _EnglishObsSkyConditionTable
import SkyConditionCodes
_SkyConditionInfo = _SkyConditionInfo
import SkyConditionCodes
_SpanishFcstSkyConditionTable = _SpanishFcstSkyConditionTable
import SkyConditionCodes
_SpanishObsSkyConditionTable = _SpanishObsSkyConditionTable
import SkyConditionCodes
...
def getValidImageList(dataPath, productString):
    ...
    for filename in imageList:
        ...
        try:
            issueTime = long(fileParts[4])
            expirationTime = long(fileParts[5])
        except:
            issueTime = 1
            expirationTime = 1
...
def playVocal(utcTimeStamp, offTimes):
    ...
    for offTimeTuple in offTimes:
        ...
        if startHour < endHour:
            if easternHour <= easternHour:
                pass
            elif easternHour <= endHour:
